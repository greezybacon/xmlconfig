...
class Blowfish:
    ...
    def encrypt (self, data):
        ...
        while i < len(data):
            ...
            xl = data[i+3] | (data[i+2] << 8) | (data[i+1] << 16) | (data[i] << 24)
            xr = data[i+7] | (data[i+6] << 8) | (data[i+5] << 16) | (data[i+4] << 24)
    ...
    def decrypt (self, data):
        ...
        while i < len(data):
            # Use big endianess since that's what everyone else uses
            cl = data[i+3] | (data[i+2]) << 8 | (data[i+1]) << 16 | (data[i]) << 24
            cr = data[i+7] | (data[i+6]) << 8 | (data[i+5]) << 16 | (data[i+4]) << 24
